<div class="d-flex justify-content-between align-items-center mb-4 flex-wrap gap-2">
    <div>
        ...
        <h4 class="fw-bold mb-1 text-dark">Edit Society director</h4>
        <p class="text-muted small mb-0">Update personal details and directorship settings.</p>
    </div>
</div>

<form action="<?= \Core\Helper::baseUrl('modules/directors/update'); ?>" method="POST" id="editForm">
    <?= \Core\CSRF::getFormField(); ?>
    <input type="hidden" name="id" value="<?= $director['id'] ?? '' ?>">
    <input type="hidden" name="party_id" value="<?= $director['party_id'] ?? '' ?>">
                ...
                <div class="card-body pt-0">
                    <div class="row g-3">
                        <div class="col-md-4">
                            ...
                            <input type="text" class="form-control fw-bold text-success" name="member_no" required value="<?= htmlspecialchars($director['member_no'] ?? '') ?>">
                        </div>
                        <div class="col-md-8">
                ...
                <div class="card-body pt-0">
                    <div class="row g-3">
                        <div class="col-12">
                            ...
                            <input type="text" class="form-control" name="heir_name" placeholder="Full name of the heir" value="<?= htmlspecialchars($director['heir_name'] ?? '') ?>">
                        </div>
                        <div class="col-md-6">
                            <label class="form-label fw-semibold small">Heir NIC</label>
                            <input type="text" class="form-control" name="heir_nic" placeholder="Heir's NIC number" value="<?= htmlspecialchars($director['heir_nic'] ?? '') ?>">
                        </div>
                        <div class="col-md-6">
                            <label class="form-label fw-semibold small">Heir Contact Number</label>
                            <input type="text" class="form-control" name="heir_contact_number" placeholder="Phone number" value="<?= htmlspecialchars($director['heir_contact_number'] ?? '') ?>">
                        </div>
                        <div class="col-12">
                            <label class="form-label fw-semibold small">Heir Address</label>
                            <input type="text" class="form-control" name="heir_address" placeholder="Heir's address" value="<?= htmlspecialchars($director['heir_address'] ?? '') ?>">
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="card-body pt-0">
                    <div class="row g-3">
                        <div class="col-12">
                            ...
                            <textarea class="form-control" name="notes" rows="3" placeholder="Additional cooperative notes..."><?= htmlspecialchars($director['notes'] ?? '') ?></textarea>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="card border-0 shadow-sm rounded-4 mt-4">
                <div class="card-body p-3 text-center">
                    <button type="submit" class="btn btn-primary rounded-pill w-100" style="background-color: #1b4332; border-color: #1b4332;">
                        <i class="bi bi-save me-1"></i> Update director
                    </button>
                </div>
            </div>
        </div>
