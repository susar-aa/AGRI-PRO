...
<div class="d-flex justify-content-between align-items-center mb-4 flex-wrap gap-2">
    <div>
        ...
        <h4 class="fw-bold mb-1 text-dark">Supplier Payment Voucher: <?= htmlspecialchars($pr['payment_number']); ?></h4>
        <p class="text-muted small mb-0">Record Date: <strong><?= htmlspecialchars($pr['payment_date']); ?></strong></p>
    </div>
    
    <div class="d-flex gap-2">
        <?php if ($pr['status'] === 'draft' && \Core\Auth::hasPermission('supplier_payments.post')): ?>
            <form action="<?= \Core\Helper::baseUrl('supplier-payments/post'); ?>" method="POST" class="d-inline">
                <?= \Core\CSRF::getFormField(); ?>
                <input type="hidden" name="id" value="<?= $pr['id']; ?>">
                <button type="submit" class="btn btn-success rounded-pill px-4" style="background-color: #1b4332; border-color: #1b4332;">
                    <i class="bi bi-send me-1"></i> Post Payment
                </button>
            </form>
        <?php endif; ?>
        <?php if ($pr['status'] === 'posted' && \Core\Auth::hasPermission('supplier_payments.reverse')): ?>
            <button class="btn btn-outline-danger rounded-pill px-4" data-bs-toggle="modal" data-bs-target="#reversePaymentModal">
    ...
    <div class="col-12 col-lg-8">
        <div class="card border-0 shadow-sm rounded-4 mb-4">
            <div class="card-header bg-white py-3 border-0 d-flex justify-content-between align-items-center">
                <h6 class="fw-bold mb-0 text-dark"><i class="bi bi-wallet2 text-success me-2"></i> Payment Voucher Details</h6>
                <div>
                    <?php
            ...
            <div class="card-body pt-0">
                <div class="row g-3">
                    ...
                    <div class="col-12 col-md-8">
                        <small class="text-muted d-block">Source Account Ledger</small>
                    ...
                    <div class="col-6 col-md-4">
                        <small class="text-muted d-block">Total Disbursement Amount</small>
                        <span class="fw-bold text-danger fs-5">LKR <?= number_format($pr['amount'], 2); ?></span>
                    </div>
    ...
    <div class="col-12 col-lg-4">
        <div class="card border-0 shadow-sm rounded-4 mb-4">
            <div class="card-header bg-white py-3 border-0">
                <h6 class="fw-bold mb-0 text-dark"><i class="bi bi-person text-success me-2"></i> Supplier Profile</h6>
            </div>
            <div class="card-body p-3 pt-0 small">
                <div class="p-3 bg-light rounded-4 mb-3 border">
                    ...
                    <a href="<?= \Core\Helper::baseUrl('parties/view?id=' . $pr['party_id']); ?>" class="btn btn-sm btn-success rounded-pill px-3 w-100">
                        <i class="bi bi-person-badge"></i> View Supplier Profile
                    </a>
                </div>
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content border-0 shadow">
            <div class="modal-header text-white bg-danger">
                <h5 class="modal-title fw-bold"><i class="bi bi-arrow-counterclockwise me-2"></i> Reverse Payment Entry</h5>
                <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
            </div>
            <form action="<?= \Core\Helper::baseUrl('supplier-payments/reverse'); ?>" method="POST">
                <?= \Core\CSRF::getFormField(); ?>
                <input type="hidden" name="id" value="<?= $pr['id']; ?>">
                <div class="modal-body p-4">
                    <p>Reversing this disbursement payment will post an offsetting ledger adjustment (Dr Cash/Bank / Cr Accounts Payable) and restore the supplier's payable outstanding balance.</p>
                    <div class="mb-3">
                        <label for="reversal_reason" class="form-label fw-semibold">Reason for Reversal <span class="text-danger">*</span></label>
                        <textarea class="form-control" id="reversal_reason" name="reversal_reason" rows="3" placeholder="Explain why this payment is being reversed..." required></textarea>
                    </div>
                </div>
